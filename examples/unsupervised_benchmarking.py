...
def main():
    ...
    print(f"Original image: {original_image.shape[0]}x{original_image.shape[1]} pixels ({original_image.shape[0] * original_image.shape[1]:,} total)")
    ...
    print(f"Working with: {image.shape[0]}x{image.shape[1]} pixels ({image.shape[0] * image.shape[1]:,} total)\n")
    ...
    for result in results:
        print(f"{result.method_name:30s} {result.execution_time:8.4f}s  "
              f"{result.seconds_per_pixel:.10f}s/px")
    
    print()
    print("Segmentation Quality Metrics (Unsupervised)")
    print("-" * 80)
    print(f"{'Method':30s} {'Objects':>8s} {'Coverage':>10s} {'Mean Area':>12s} {'Compactness':>12s}")
    print("-" * 80)
    ...
    for result in results:
        ...
        print(f"  Area stats: mean={u.mean_area:.2f}, std={u.std_area:.2f}, median={u.median_area:.2f}")
        print(f"  Area range: [{u.min_area:.2f}, {u.max_area:.2f}]")
        print(f"  Shape metrics: compactness={u.mean_compactness:.4f}, solidity={u.mean_solidity:.4f}")
    ...
    print(f"  Saving full resolution annotations (original size: {original_image.shape[0]}x{original_image.shape[1]})...")
    for result in results:
        seg.save_geojson(
            result.geojson, 
