...
class UnsupervisedClusteringSegmenter:
    ...
    def extract_patch_features(self, patch: np.ndarray) -> np.ndarray:
        ...
        if len(patch.shape) == 3 and patch.shape[2] == 3:
            ...
            gray = patch_float if len(patch_float.shape) == 2 else patch_float.mean(axis=2)
    ...
    def segment(self, image: np.ndarray) -> dict:
        ...
        for cluster_id in np.unique(labels):
            ...
            mean_intensity = np.mean(cluster_features[:, intensity_idx]) if cluster_features.shape[1] > intensity_idx else 0.5
            mean_saturation = np.mean(cluster_features[:, saturation_idx]) if cluster_features.shape[1] > saturation_idx else 0.5
