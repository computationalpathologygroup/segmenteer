...
def create_heatmap_overlay(image: np.ndarray, geojson_data: dict, alpha: float = 0.4) -> np.ndarray:
    if image.ndim == 2:
    ...
    overlay[mask] = (overlay[mask] * (1 - alpha) + green_overlay[mask] * alpha).astype(np.uint8)
